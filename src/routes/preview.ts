...
router.get('/template/:templateId', (req: Request, res: Response) => {
  ...
  res.type('html').send(renderPageHtml({ ...page, status: 'published' }, project, design, site, blocks, navigation, res.locals.cspNonce));
});
...
// Voorkomt dat custom_css uit de <style>-tag breekt (HTML-parser sluit op letterlijke '<')
function sanitizeCustomCss(css: string): string {
  return css.replace(/</g, '\\003C ');
}
...
export function renderPageHtml(page: any, project: any, design: any, site: any, blocks: PreviewBlock[], navigation: any[], nonce?: string): string {
  ...
  const customCss = site ? sanitizeCustomCss(String(design?.custom_css || '')) : '';
  const spacingUnit = escapeHtml(design?.spacing_unit || '1rem');
...
<style${nonceAttr}>
:root{--primary:${escapeHtml(design?.primary_color || '#2563eb')};--secondary:${escapeHtml(design?.secondary_color || '#0ea5e9')};--background:${escapeHtml(design?.background_color || '#ffffff')};--text:${escapeHtml(design?.text_color || '#1f2937')};--radius:${escapeHtml(design?.border_radius || '0.5rem')};--spacing:${spacingUnit};--font-family:${fontFamily};--heading-font:${headingFont};}
...
<style id="preview-custom-css"${nonceAttr}>${customCss}</style>
<script${nonceAttr}>
window.addEventListener('message', function (event) {
  if (event.origin !== window.location.origin || event.data?.type !== 'design-preview-update') return;
  var design = event.data.design || {};
  var root = document.documentElement;
  if (design.primary_color) root.style.setProperty('--primary', design.primary_color);
  if (design.secondary_color) root.style.setProperty('--secondary', design.secondary_color);
  if (design.background_color) root.style.setProperty('--background', design.background_color);
  if (design.text_color) root.style.setProperty('--text', design.text_color);
  if (design.border_radius) root.style.setProperty('--radius', design.border_radius);
  if (design.font_family) root.style.setProperty('--font-family', design.font_family);
  if (design.heading_font_family) root.style.setProperty('--heading-font', design.heading_font_family);
  document.body.style.fontFamily = design.font_family || '';
  document.querySelectorAll('h1,h2,h3,h4,h5,h6').forEach(function (heading) { heading.style.fontFamily = design.heading_font_family || ''; });
  var customCss = document.getElementById('preview-custom-css');
  if (customCss) customCss.textContent = design.custom_css || '';
});
</script>
</head>
...
router.get('/:projectSlug/:pageSlug?', (req: Request, res: Response) => {
  ...
  res.type('html').send(renderPageHtml(page, project, design, site, blocks, navigation, res.locals.cspNonce));
});
